...
<div class="container-fluid px-4">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h3 class="fw-bold text-dark mb-1">Halo, {{ explode(' ', auth()->user()->name)[0] }}! 👋</h3>
            ...
            <div class="card border-0 shadow-sm h-100 hover-up">
                <div class="card-body p-4">
                    <div class="d-flex justify-content-between align-items-start">
                        <div>
                            <p class="text-muted fw-semibold small mb-2">{{ $c['title'] }}</p>
                            <h3 class="fw-bold mb-0" style="color: #1e293b;">{{ $c['val'] }}</h3>
                        </div>
                        <div class="stats-icon-wrapper" style="color: {{ $c['color'] }}">
                            <i class="bi {{ $c['icon'] }}"></i>
                        </div>
                    </div>
                </div>
        ...
        <div class="col-lg-4">
            <div class="card border-0 shadow-sm h-100">
                <div class="card-header py-4 px-4 bg-white border-0 d-flex justify-content-between align-items-center">
                    ...
                    <a href="{{ route('admin.orders.index') }}" class="text-decoration-none small fw-bold" style="color: var(--accent-color)">Lihat Semua</a>
                </div>
                <div class="card-body px-3 pt-0">
                    <div class="overflow-auto" style="max-height: 400px;">
                        ...
                        <div class="d-flex align-items-center p-3 order-item bg-light bg-opacity-50">
                            ...
                            <div class="text-end">
                                <div class="fw-bold text-dark small">Rp{{ number_format($order->total_amount, 0, ',', '.') }}</div>
                                ...
                                <span class="badge rounded-pill bg-opacity-10 {{ $currentStatus['class'] }}" style="font-size: 0.65rem;">
                                    {{ $currentStatus['label'] }}
                                </span>
                            </div>
                        </div>
            ...
            <div class="row g-4">
                ...
                <div class="col-6 col-md-4 col-lg-2">
                    <div class="product-grid-item text-center">
                        <img src="{{ $product->image_url }}" class="top-product-img mb-3" alt="{{ $product->name }}">
                        <h6 class="text-truncate fw-bold mb-1 small">{{ $product->name }}</h6>
                        <div class="d-flex justify-content-center align-items-center gap-2">
                            <span class="badge rounded-pill px-2" style="font-size: 0.6rem; background: #6366f1;">{{ $product->sold }} Terjual</span>
                        </div>
                    </div>
                </div>
                @endforeach
